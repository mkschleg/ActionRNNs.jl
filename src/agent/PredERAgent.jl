...
function MinimalRLCore.step!(agent::PredERAgent, env_s_tp1, r, terminal, rng; kwargs...)
    ...
    # RNN update function
    # println(length(agent.replay), agent.warm_up)
    ℒ = 0.0f0
    if length(agent.replay) >= agent.warm_up
        ...
        state_list = if eltype(agent.state_list) isa Tuple
            sl = [(exp[i].am1, exp[i].s) for i in 1:length(exp)]
            push!(sl, (exp[end].a, exp[end].sp))
        else
