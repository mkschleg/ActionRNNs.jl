...
@testset "Consistency.DirTMazeER" begin
    ...
    @testset "AARNN" begin
        ret = @run_experiment DirectionalTMazeERExperiment  "AARNN" DTMAZE_ER_BASE_CONFIG
        @test dir_tmaze_check(ret, -1465.8024f0)
    end

    @testset "MARNN" begin
        ret = @run_experiment DirectionalTMazeERExperiment  "MARNN" DTMAZE_ER_BASE_CONFIG
        @test dir_tmaze_check(ret, -1157.3124f0)
    end

    @testset "FacMARNN" begin
        ret = @run_experiment DirectionalTMazeERExperiment  "FacMARNN" DTMAZE_ER_BASE_CONFIG factors=8 init_style="tensor"
        @test dir_tmaze_check(ret, -1352.9984f0)
    end

    @testset "GRU" begin
        ret = @run_experiment DirectionalTMazeERExperiment  "GRU" DTMAZE_ER_BASE_CONFIG
        @test dir_tmaze_check(ret, -849.3139f0)
    end

    @testset "AAGRU" begin
        ret = @run_experiment DirectionalTMazeERExperiment  "AAGRU" DTMAZE_ER_BASE_CONFIG
        @test dir_tmaze_check(ret, -1297.6978f0)
    end

    @testset "MAGRU" begin
        ret = @run_experiment DirectionalTMazeERExperiment  "MAGRU" DTMAZE_ER_BASE_CONFIG
        @test dir_tmaze_check(ret, -1173.9962f0)
    end

    @testset "FacMAGRU" begin
        ret = @run_experiment DirectionalTMazeERExperiment  "FacMAGRU" DTMAZE_ER_BASE_CONFIG factors=8 init_style="tensor"
