...
# ╔═╡ cb0cce4a-3687-41d3-8cb8-dafc6d2e19f9
@bind s_dir PlutoUI.Select(readdir(at(".")))

# ╔═╡ 4e5fef7e-86e2-49a2-aa43-ec888e7c83e3
readdir(at(s_dir))

# ╔═╡ 8a7989fb-063b-46dc-af95-b0ef5d293496
df = FileIO.load(at("ringworld_er_deep_a_v2/2022_05_18_proc_data.jld2"))["params_and_results"]
...
let
	...
	get_cell_data = (cell, nh, ial) -> begin
	...
	plts = []
	for nal ∈ sort(diff_dict["internal_a_layers"])
		plt = plot(
			# title="Experts=$(ne)", 
			xlabel="NumHidden", 
			ylabel="MAVE",
		    title="DeepAction Layers: $(nal)")
		for (cell, nh, _) ∈ [["AARNN", 4, 4], ["AARNN", 12, 2], ["AAGRU", 9, 10]]
			cd = get_cell_data(cell, nh, nal)
			plot!(plt, cd[!, :x], cd[!, :μ], yerr=cd[!, :σ], label=(cell, nh))
		end
		push!(plts, plt)
	end
	plot(plts...)
...
# ╔═╡ daaca141-84d3-4374-9da5-ce8cd6664cd2
let
	...
	get_cell_data = (cell, nh, ial, τ) -> begin
		@from i in df begin
			@where i.cell == cell &&
				   i.replay_size == replay_size &&
				   i.numhidden == nh &&
	...
	for (τ, nal) ∈ Iterators.product(sort(diff_dict["truncation"]), 
							         sort(diff_dict["internal_a_layers"]))
		plt = plot(
			# title="Experts=$(ne)", 
			xlabel="NumHidden",
			ylabel="MAVE",
		    title="DeepAction Layers: $(nal), truncation = $(τ)")
		for (cell, nh, _) ∈ [["AARNN", 4, 4], ["AARNN", 12, 2], ["AAGRU", 9, 10]]
			cd = get_cell_data(cell, nh, nal, τ)
			plot!(plt, cd[!, :x], cd[!, :μ], yerr=cd[!, :σ], label=(cell, nh))
		end
		push!(plts, plt)
	end
...
# ╔═╡ 5206c2a8-0265-4bcf-96fe-a5f47df30c66
DataFrameUtils.get_diff_dict(best_over_eta)

# ╔═╡ 0f04e8ab-ec34-479f-aedc-757ca5158c83
let
	...
	args = Dict("args"=>[Dict(names(row) .=> values(row)) for row in eachrow(_df)])
	# FileIO.save("../../final_runs/ringworld_deep_action.jld2", args)
end

# ╔═╡ a2b51975-21ff-4987-985c-02a8838f201f
FileIO.load("../../final_runs/ringworld_deep_action.jld2")
