...
# ╔═╡ c33c621e-d84f-11ec-3859-bfd8a4d9e50f
begin
...
# ╔═╡ 55358cec-faf3-46e7-ae19-851f3a4c3ac4
cell_colors = Dict(
	"RNN" => color_scheme[3],
...
df_final = FileIO.load(at("final_ringworld_er_rmsprop_10/2022_05_20_proc_data.jld2"))["params_and_results"]

# ╔═╡ 9c20f297-b6e3-4644-b97e-25b0809465f6
df_fac = let
	dirs = [
		"ringworld_fac_er_rmsprop_10", 
		"ringworld_fac_er_rmsprop_10_marnn",
		"ringworld_fac_er_rmsprop_10_aagru",
		"ringworld_fac_er_rmsprop_10_magru"
	]
	fboe(d) = DataFrameUtils.best_from_sweep_param(
		order(:avg_end, by=(x)->mean(x[1]), rev=false), 
		FileIO.load(at("$(d)/2022_05_20_proc_data.jld2"))["params_and_results"], 
		["eta"])
	d = [fboe(d) for d in dirs]
	append!(d[1], d[2])
	append!(d[1], d[3])
	append!(d[1], d[4])
	d[1]
...
df_deep_action = FileIO.load(at("ringworld_er_deep_a_v2/2022_05_18_proc_data.jld2"))["params_and_results"]

# ╔═╡ 7bde35d3-6e81-48c1-90fe-6226e811e885
df_deep_action_boe = DataFrameUtils.best_from_sweep_param(
...
let
	cell = "FacMARNN"
	nh = 9
	save_dir = "/home/mkschleg/scratch/ActionRNNs/ringworld_fac_er_rmsprop_10_marnn/data"
	cd = @from i in df_fac begin
			@where i.cell == cell &&
				   i.save_dir == save_dir &&
				   i.numhidden == nh
			...
			@select {
				x = i.truncation,
				μ = mean(i.avg_end[1]),
				σ = sqrt(var(i.avg_end[1])/length(i.avg_end[1]))
			}
			@collect DataFrame
		end
end
...
let
	...
	for (cell, nh) ∈ [("MARNN", 12), ("RNN", 15), ("AARNN", 15)]
	...
	cell = "FacMARNN"
	nh = 15
	save_dir = "/home/mkschleg/scratch/ActionRNNs/ringworld_fac_er_rmsprop_10/data"
	cd = @from i in df_fac begin
			@where i.cell == cell &&
				   i.save_dir == save_dir &&
				   i.numhidden == nh
	...
	nh = 12
	ial = 1
	cd = @from i in df_deep_action_boe begin
			@where i.cell == cell &&
				   i.numhidden == nh &&
				   i.internal_a_layers == ial
			@orderby ascending(i.truncation)
			@select {
				x = i.truncation,
				μ = mean(i.avg_end),
				σ = sqrt(var(i.avg_end)/length(i.avg_end))
			}
			@collect DataFrame
		end
	...
	# savefig(plt, "../../plots/ringworld_sens_rnn.pdf")
	plt
end
...
let
	...
	for (cell, nh) ∈ [("MAGRU", 9), ("GRU", 12), ("AAGRU", 12)]
		cd = @from i in df_final begin
			@where i.cell == cell &&
				   i.numhidden == nh
			@orderby ascending(i.truncation)
	...
	cell = "FacMAGRU"
	nh = 12
	save_dir = "/home/mkschleg/scratch/ActionRNNs/ringworld_fac_er_rmsprop_10_aagru/data"
	cd = @from i in df_fac begin
			@where i.cell == cell &&
				   i.save_dir == save_dir &&
				   i.numhidden == nh
	...
	cell = "FacMAGRU"
	nh = 9
	save_dir = "/home/mkschleg/scratch/ActionRNNs/ringworld_fac_er_rmsprop_10_magru/data"
	cd = @from i in df_fac begin
			@where i.cell == cell &&
				   i.save_dir == save_dir &&
				   i.numhidden == nh
	...
	# savefig(plt, "../../plots/ringworld_sens_gru.pdf")
	plt
end
...
function stderr_uneven(d::Vector{Array{F, 1}}; z=1.0) where {F}
...
let
	...
	for (cell, nh) ∈ [("MARNN", 9), ("RNN", 15), ("AARNN", 15)]
		...
		y = [rollmean(lc, 10) for lc in cd[1, :y]]
		μ = mean_uneven(y )
		σ = stderr_uneven(y, z=1.97)
		plot!(plt, μ, ribbon=σ,
			lw=3, palette=color_scheme, 
			color=cell_colors[cell])
	end
	savefig("../../plots/ringworld_rnn_lc.pdf")
	plt
end
