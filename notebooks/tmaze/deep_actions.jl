...
df_multil = FileIO.load(at("tmaze_er_deep_action/2022_05_19_proc_data.jld2"))["params_and_results"]
...
let
	...
	get_cell_data = (cell, ial) -> begin
		@from i in best_over_eta_multil begin
			...
			@orderby ascending(i.internal_a)
			@select {
				x = i.internal_a, 
...
let
	...
	replay_size = 1000
	plot_data_sym = :successes_avg_end
	get_cell_data(cell, ia, ial) = begin
	...
	for ial ∈ diff_dict["internal_a_layers"]
		...
		for (cell, ia) ∈ Iterators.product(diff_dict["cell"], diff_dict["internal_a"])
			cd = get_cell_data(cell, ia, ial)
			plot!(plt, 
				cd[!, :x], 
				cd[!, :μ], 
				yerr=cd[!, :σ], 
				label=string((cd[1, :cell], ia)))
		end
		push!(plts, plt)
	end
	plot(plts...)
...
# ╔═╡ 24cdd63c-ea0b-46a6-80d4-5236752df363
let
	# Plot over sweeps
	plotly()
	diff_dict = DataFrameUtils.get_diff_dict(df_multil)					
	...
	plts = []
	for (ial, ia) ∈ Iterators.product(diff_dict["internal_a_layers"], sort(diff_dict["internal_a"]))
...
# ╔═╡ 7c724609-c8c6-4122-9abb-26990ecb293e
let
...
# ╔═╡ 10bc2e12-21cc-4da2-8b84-9aac7b3ce40f
FileIO.load("../../final_runs/tmaze_deep_action.jld2")
