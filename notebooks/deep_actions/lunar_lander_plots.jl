...
data = RPU.get_line_data_for(
	...
	get_data=(x)->RPU.get_rolling_mean_line(x, :total_steps, 5000))
...
let
	boxplot(data_dist, Dict("numhidden"=>64, "truncation"=>12, "cell"=>"MAGRU"); 
		label = "DMAGRU",
		color=cell_colors["MAGRU"],
		legend=false, lw=1.5, ylims=(0.4, 1.0), tickdir=:out, grid=false)
	dotplot!(data_dist, Dict("numhidden"=>64, "truncation"=>12, "cell"=>"MAGRU"); 
		label = "DMAGRU",
		color=cell_colors["MAGRU"])
	
	boxplot!(data_dist, Dict("numhidden"=>152, "truncation"=>12, "cell"=>"AAGRU"); 
		label = "DAAGRU",
		color=cell_colors["AAGRU"],
		legend=false, lw=1.5, ylims=(0.4, 1.0), tickdir=:out, grid=false)
	dotplot!(data_dist, Dict("numhidden"=>152, "truncation"=>12, "cell"=>"AAGRU"); 
		label = "DAAGRU",
		color=cell_colors["AAGRU"])
