...
df = FileIO.load(at("dir_tmaze_er_10_deep_action/2022_05_12_processed_data.jld2"))["params_and_results"]
...
let
	# τ = 15
	best_over_ = DataFrameUtils.best_from_sweep_param(
		order(:successes_avg_end, by=mean, rev=true), 
		df, 
		["eta", "numhidden", "internal_a"])
	replay_size = 10000
	plot_data_sym = :successes_avg_end
	
	gr()
	plts = []
	diff_dict = DataFrameUtils.get_diff_dict(best_over_)


	get_cell_data = (cell) -> begin
		@from i in best_over_ begin
			@where i.cell == cell &&
				   i.replay_size == replay_size
			@orderby ascending(i.numhidden)
			@select {
				x = i.cell, 
				μ = getindex(i, plot_data_sym)}
			@collect DataFrame
		end
	end
	...
	for cell ∈ diff_dict["cell"]
		cd = get_cell_data(cell)
		boxplot!(plt, cd[!, :x], cd[!, :μ], label=cell)
	end
	plt
		# push!(plts, plt)
	# end
...
# ╔═╡ ea6ef6f9-d488-4ec7-92a8-ae661e570c61
md"""
# Multi Layer
"""

# ╔═╡ 8717553c-2e54-47c0-bcac-d2e6bd720ba8
df_multil = FileIO.load(at("dir_tmaze_er_10_deep_action_multil_fixed/2022_05_19_proc_data.jld2"))["params_and_results"]

# ╔═╡ 2cec3363-cec8-42d2-a308-3b04c6fb2067
best_over_eta_multil = DataFrameUtils.best_from_sweep_param(
	order(:successes_avg_end, by=mean, rev=true), 
	df_multil, 
	["eta"])

# ╔═╡ 5f5ecd90-ac3c-4947-8486-1b2a12c01ba7
DataFrameUtils.get_diff_dict(best_over_eta_multil)
...
let
	...
	gr()
	plts = []
	diff_dict = DataFrameUtils.get_diff_dict(best_over_eta_multil)


	get_cell_data = (cell, ial) -> begin
		@from i in best_over_eta_multil begin
			@where i.cell == cell &&
				   i.replay_size == replay_size &&
	...
	plts=[]
	for ial ∈ sort(diff_dict["internal_a_layers"])
		plt = plot(
			title="Action Layers=$(ial)", 
			xlabel="NumHidden",
			ylabel="Average Successes")
		for cell ∈ diff_dict["cell"]
...
let
	# Plot over sweeps
	plotly()
	diff_dict = DataFrameUtils.get_diff_dict(df_multil)					
	replay_size = 10000
	plot_data_sym = :successes_avg_end
	get_cell_data(nh, ial) = begin
		@from i in df_multil begin
	...
	plts = []
	for ial ∈ diff_dict["internal_a_layers"]
		...
		for nh ∈ diff_dict["numhidden"]
			cd = get_cell_data(nh, ial)
			plot!(plt, 
				cd[!, :x], 
				cd[!, :μ], 
				yerr=cd[!, :σ], 
				label=string((cd[1, :cell], nh)))
		end
		push!(plts, plt)
	end
	plot(plts...)
...
# ╔═╡ 3da76142-641b-4bde-9ac9-315286f0adf2
let
	bdf = DataFrameUtils.best_from_sweep_param(
		order(:successes_avg_end, by=mean, rev=true), 
		df_multil, 
		["eta"])

	df = @from i in bdf begin
		@where true
		@select {
			i.cell, 
			i.replay_size,
	...
	# FileIO.save("../../final_runs/dirtmaze_deep_action.jld2", args)
end

# ╔═╡ 2f12b3c1-4007-4dbe-9865-319ea8c0f34a
FileIO.load("../../final_runs/dirtmaze_deep_action.jld2")
