...
# ╔═╡ 9cdf595e-e977-4e1b-830b-11b14d877f24
ic_dir_10 = let
	ic_arr = ItemCollection.(at.(["dir_tmaze_er_rnn_rmsprop_10/", 
     "dir_tmaze_er_rnn_rmsprop_10_p2/"]))
	ItemCollection(vcat(getfield.(ic_arr, :items)...))
end

# ╔═╡ bbac0837-95db-4c77-9429-a5cf322bd335
...
# ╔═╡ 30b7f95d-4f09-490c-b628-73c7475cb950
fieldnames(typeof(data_10.data[1]))

# ╔═╡ a40d5f2b-3978-4597-8bea-b8e32474f0b7
data_10.data[1].data_pms

# ╔═╡ 5f6886fc-57b6-4f58-bc29-236c272b5d41
...
# ╔═╡ f1543732-32e8-4458-8dda-23da94424e2e
df_data_10 = datacol_to_df(data_10, ["eta"])

# ╔═╡ 518862ab-0711-465a-82a9-fd2a419599c5
df_data_10[(df_data_10.cell .== "GRU") .& (df_data_10.truncation .== 12), :]
...
function ic_to_df(ic::Reproduce.ItemCollection; filter=nothing)
...
function files_to_ic_to_df(files::Vector{String}; filter=nothing)
	ic_arr = [ItemCollection(at(file)) for file in files]
	ic_to_df(ItemCollection(vcat(getfield.(ic_arr, :items)...)); filter=filter)
end

# ╔═╡ 5a03146e-5264-423b-ad14-b0dae69b0fff
df_10 = files_to_ic_to_df(["dir_tmaze_er_rnn_rmsprop_10/", 
		           		"dir_tmaze_er_rnn_rmsprop_10_p2/"]; 
	filter=["_GIT_INFO", "_SAVE", "save_dir"])
...
function load_runs(df::DataFrame, get_data)
	...
	diff_dict = Dict(diff_arr[[(length(a[2]) > 1) & (a[1] ∉ ["_HASH", "folder_str"]) for a in diff_arr]])
	
    pms_template = @static if VERSION <= v"1.5"
        (; Symbol(k)=>df[1, k] for k ∈ keys(diff_dict))
    else
    ...
    for (idx, row) ∈ enumerate(eachrow(df))
        ...
        @static if VERSION <= v"1.5"
            push!(pms, (; Symbol(k)=>row[k] for k ∈ keys(diff_dict)))
        else
            push!(pms, NamedTuple(Symbol(k)=>row[k] for k ∈ keys(diff_dict)))
        end
    end
    d, pms
end
...
function find_best_params(df::DataFrame,
    ...
    if comp isa Symbol
        comp = RPU.get_comp_function(comp)
    end
    ...
    for (p_idx, p) ∈ enumerate(params)
        ...
        s[p_idx] = mean(load_runs(df_view, get_comp_data)[1])
    end
    ...
    data, data_pms, v, params[idx], s, params
...
function get_line_data_for(
	...
	params = if line_keys isa String
		unique(sort!(df[:, line_keys]))
	else
		Iterators.product([unique(sort!(df[:, k])) for k in line_keys]...)
	end
    ...
    Threads.@threads for p_idx ∈ 1:length(params)
		...
		df_view = filter(df; view=false) do row
			all(df[line_keys[i]] == p[i] for i ∈ 1:length(p))
		end

		if !isempty(sub_ic)
			d, dp, c, c_p, s, pms = find_best_params(
				sub_ic,
				param_keys,
				comp,
				get_comp_data,
			...
			d = Dict(line_keys[i]=>params[p_idx][i] for i in 1:length(line_keys))
			merge!(d, Dict(param_keys[i]=>ps[i] for i in 1:length(param_keys)))
			d["data"] = d
			d["data_params"] = dp
			d["comp"] = c
			lock(lk) do
...
# ╔═╡ 120b16cc-02fb-4ba0-944c-4051bd95e164
test_df = df_10[(df_10.cell .== "GRU") .& (df_10.numhidden .== 20) .& (df_10.truncation .== 10), :]

# ╔═╡ 88618fee-ec1e-4108-82cd-05f4825a4a9c
find_best_params(test_df, 
	["eta"],
	findmax,
	(x)->RPU.get_MUE(x, :successes),
	(x)->RPU.get_rolling_mean_line(x, :successes, 300)) 

# ╔═╡ ac855cc7-dc26-44c9-a01a-fb48f0b83673
filter(test_df) do row
	(row["eta"] == 0.00125) & (row["seed"] == 1)
end

# ╔═╡ 813c95ca-ec16-4d0b-8309-405cbc5372ff
md"""
# Details
"""

# ╔═╡ 1fde233a-7665-406b-9829-d90879d394e5
color_scheme = [
    colorant"#44AA99",
    colorant"#332288",
    colorant"#DDCC77",
...
ReproducePlotUtils = "c4339a9b-a0bc-4ed5-8923-5e6db10394ac"
...
[[Reproduce]]
deps = ["ArgParse", "BSON", "CodeTracking", "Dates", "Distributed", "FileIO", "Git", "HDF5", "JLD2", "JSON", "Logging", "Parallelism", "Pkg", "ProgressMeter", "Random", "Reexport", "SharedArrays", "Sockets"]
git-tree-sha1 = "c8d07aa38f4569f6937d0a5a1763cfc479a4ab26"
uuid = "560a9c3a-0b8c-11e9-0329-d39dfcb85ed2"
version = "0.9.6"

[[ReproducePlotUtils]]
deps = ["DataFrames", "FileIO", "PlotUtils", "Plots", "RecipesBase", "Reproduce", "RollingFunctions", "Statistics", "StatsPlots"]
git-tree-sha1 = "f83fa9f3a037081608e4607dc7582913c2e19f3f"
repo-rev = "master"
repo-url = "https://github.com/mkschleg/ReproducePlotUtils.jl.git"
uuid = "c4339a9b-a0bc-4ed5-8923-5e6db10394ac"
version = "0.1.0"
