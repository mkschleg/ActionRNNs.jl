...
begin
	df_deep_action = FileIO.load(at("final_lunar_lander_deep_a/2022-06-20-procdata.jld2"))["params_and_results"]
	df_MAGRU_all = FileIO.load(at("final_lunar_lander_er_relu_rmsprop_os6_sc2_magru_4M/2022_05_21_proc_data.jld2"))["params_and_results"]
	df_MAGRU = DataFrameUtils.best_from_sweep_param(
		order(:rews_avg_end, by=mean, rev=true), 
		df_MAGRU_all, 
		["eta"])
	df_AAGRU = FileIO.load(at("final_lunar_lander_er_relu_rmsprop_os6_sc2_aagru_4M/2022_05_21_proc_data.jld2"))["params_and_results"]
	df_GRU = FileIO.load(at("final_lunar_lander_er_relu_rmsprop_os6_sc2_gru_4M/2022_05_21_proc_data.jld2"))["params_and_results"]
	df_FACGRU = vcat(
		FileIO.load(at("final_lunar_lander_er_relu_rmsprop_os6_sc2_facmagru64_4M/2022_05_21_proc_data.jld2"))["params_and_results"],
		FileIO.load(at("final_lunar_lander_er_relu_rmsprop_os6_sc2_facmagru100_4M/2022_05_21_proc_data.jld2"))["params_and_results"],
		FileIO.load(at("final_lunar_lander_er_relu_rmsprop_os6_sc2_facmagru152_4M/2022_05_21_proc_data.jld2"))["params_and_results"]
	)
...
best_over_eta_deep_action[!, :eta][1]
...
let
	...
	# savefig(plt, "../../plots/lunar_lander_boxplot.pdf")
...
# ╔═╡ 143a5f4d-2a35-402c-a0e7-ebc5f54d72e3
...
function get_cumulative_line(res, steps; n=0)
    ret = zeros(eltype(res), sum(steps))
    cur_idx = 1
    for i in 1:length(res)
        ret[cur_idx:(cur_idx + steps[i] - 1)] .= res[i]
        cur_idx += steps[i]
    end
    ...
    if n == 0
		...
		return_arr = zeros(eltype(res), Int(floor(length(ret)/n)))
...
# ╔═╡ 93b3f026-1eee-4fc6-a636-328a64be3df9
function get_extended_line(res, steps; n=0)
    ret = zeros(eltype(res), sum(steps))
    cur_idx = 1
    for i in 1:length(res)
        ret[cur_idx:(cur_idx + steps[i] - 1)] .= res[i]
