...
# ╔═╡ d00fe63a-8f66-11ec-1c6b-bfc81ca6de95
begin
...
# ╔═╡ 3adc34a5-242b-464d-8214-63bac514a6cd
using StatsPlots

# ╔═╡ 21286be0-e3e6-407c-b7ab-5e7ec7a953e3
...
# ╔═╡ 5ab45575-cba3-49bf-867b-c77f47060348
...
# ╔═╡ bf287c96-f902-4d25-bf9f-b582ce563772
DataFrameUtils = ingredients(path="../modules/DataFrameUtils.jl").DataFrameUtils

# ╔═╡ 8900bd63-5fbd-4bac-9fd5-cc127578dc09
df = FileIO.load("../../data/masked_gw_baseline_v2/data_28_2_2022.jld2")["params_and_results"]

# ╔═╡ 43b29570-507a-4e8a-b42d-59f257cefdb5
sort(DataFrameUtils.get_diff_dict(df)["eta"])

# ╔═╡ 53fa36d6-d9f3-4124-95a6-ba6dfd66e580
names(df)

# ╔═╡ d338a45f-495c-4096-a8da-ce9e22f1e1fe
best_over_eta = DataFrameUtils.best_from_sweep_param(
	order(:stps_avg_end, by=mean, rev=false), 
	df, 
	["eta"])

# ╔═╡ 03eecb06-1cb7-4092-bb3e-397189ac0469
diff_dict_best_over_eta = DataFrameUtils.get_diff_dict(best_over_eta)

# ╔═╡ daf0c0ad-bd2a-4feb-8833-a2f292215e06
let
	plotly()
	plts = []
	diff_dict = DataFrameUtils.get_diff_dict(best_over_eta)
	plot_data_sym = :test_stps_avg
	for na ∈ sort(diff_dict["num_anchors"])
		x = [@from i in best_over_eta begin
			@where i.num_anchors == na && 
				   i.gamma == 0.99 &&
				   i.replay_size == 50000 &&
				   i.batch_size == 16 &&
				   i.numhidden == 25 &&
			...
			@collect DataFrame
		end for cell ∈ ["AARNN", "MARNN", "RNN"]]
		plt = plot(x[1][!, :x], x[1][!, :μ], yerr=x[1][!, :σ], 
			label="AARNN", title="Anchors=$(na)",
			grid = false, tick_dir=:out)
		
		plot!(x[2][!, :x], x[2][!, :μ], yerr=x[2][!, :σ], label="MARNN")
		plot!(x[3][!, :x], x[3][!, :μ], yerr=x[3][!, :σ], label="RNN")
		push!(plts, plt)
	end
	plt = plot(plts...)
	# savefig(plt, "masked_gw_prelim.pdf")
	# DataFrameUtils.get_diff_dict(x)
end

# ╔═╡ 982a7f77-55a5-4942-87ee-d91aa8c5f1c1
let
	plotly()
	plts = []
	diff_dict = DataFrameUtils.get_diff_dict(best_over_eta)
