...
#     ses = parsed["state_encoding_size"]
    ...
    action_chain = Flux.Chain(
        (a)->Flux.onehotbatch(a, 1:na),
        Flux.Dense(na, es, Flux.relu, initW=init_func),
        Flux.Dense(es, es, Flux.relu, initW=init_func))
    
    obs_chain = Flux.Chain(Flux.Dense(fs, es, Flux.relu; initW=init_func),
                           Flux.Dense(fs, es, Flux.relu; initW=init_func))
    Flux.Chain(
        ActionRNNs.DualStream(
            action_chain, obs_chain
        ),
        rnn_layer,
